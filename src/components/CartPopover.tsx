...
const CartPopover: React.FC<CartPopoverProps> = ({ cart, open, onClose, handleRemoveFromCart, anchorRef }) => {
  ...
  const { updateWeight, cart: cartItems } = useCart();
  ...
  // Reset estimatedOutputs when cartItems or buyAmount changes
  useEffect(() => {
    setEstimatedOutputs({});
  }, [cartItems, buyAmount, buyCurrency]);

  // Live quote fetching for estimated outputs
  useEffect(() => {
    if (!publicKey || !cartItems.length || !buyAmount || isNaN(parseFloat(buyAmount)) || parseFloat(buyAmount) <= 0) {
      setEstimatedOutputs({});
      return;
    }
    const fetchQuotes = debounce(async () => {
      const jupiter = new JupiterSwapService(connection);
      const newEstimates: { [mint: string]: string } = {};
      for (const item of cartItems) {
        ...
        try {
          ...
          if (bestRoute && bestRoute.outAmount) {
            const outputDecimals = item.token.decimals ?? (item.token.symbol === 'SOL' ? 9 : item.token.symbol === 'USDC' ? 6 : 6);
            const estimated = (parseFloat(bestRoute.outAmount) / Math.pow(10, outputDecimals)).toFixed(6);
            newEstimates[item.token.mint] = estimated;
          }
        } catch (err) {
          // Ignore errors for now, don't set estimate
        }
      }
      setEstimatedOutputs(newEstimates);
    }, 300);
    fetchQuotes();
    return () => fetchQuotes.cancel();
  }, [buyAmount, buyCurrency, cartItems, connection, publicKey]);
  ...
  return (
    <div ref={popoverRef} className="absolute right-0 top-full mt-2 w-[180%] bg-gray-900 border border-gray-700 rounded-lg shadow-lg z-50 p-4 max-h-[400px] overflow-y-auto">
      <h3 className="text-white font-semibold mb-2">Cart</h3>
      {cartItems.length === 0 ? (
        <p className="text-gray-400 text-sm">No tokens in cart.</p>
      ) : (
              ...
              <li className="flex items-center justify-between py-2">
                <div className="flex items-center flex-1 min-w-0 space-x-2">
                  <img
                    ...
                    className="w-6 h-6 rounded-full object-cover border border-gray-600 bg-gray-700 flex-shrink-0"
                    onError={e => {
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    {estimatedOutputs[item.token.mint] && (
                      <span className="text-xs text-green-400 mt-0.5">≈ {estimatedOutputs[item.token.mint]} {item.token.symbol}</span>
                    )}
                  </div>
                </div>
      ...
      {buyResult === 'success' && (
        <div className="mt-2 text-sm text-green-400">Buy successful!</div>
      )}
    </div>
  );
};
